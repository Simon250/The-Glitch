...
bool Scene0::OnCreate() {
	...
	if (ObjLoader::loadOBJ("meshes/ViperInYourFace.obj") == false) {
		return false;
	}
	...
	demoObject->setModelMatrix(MMath::translate(demoObject->getPos()) * MMath::rotate(180.0f, Vec3(0.0f, 1.0f, 0.0f)));
...
void Scene0::Update(const float deltaTime) {
	...
	rotation += 1.0f;
	demoObject->setModelMatrix(MMath::rotate(rotation, Vec3(1.0f, 0.0f, 0.0f)));
	
	demoObject->setVel(Vec3(1.0, 0.0, 0.0));
	Physics::SimpleNewtonMotion(*demoObject, deltaTime);
	demoObject->setModelMatrix(MMath::translate(demoObject->getPos()) * MMath::rotate(rotation, Vec3(1.0f, 0.0f, 0.0f)) * MMath::rotate(180.0f, Vec3(0.0f, 1.0f, 0.0f)));
}
